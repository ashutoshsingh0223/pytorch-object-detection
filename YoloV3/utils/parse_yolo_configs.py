    ...
    for index, block in enumerate(blocks[1:]):
        ...
        if type_ == "convolutional":
            ...
            module_block.add_module(
                f"upsample_{index}", nn.Upsample(scale_factor=stride, mode="bilinear")
            )
            ...
            if end < 0:
                out_channels = (
                    out_channels_list[start + index] + out_channels_list[end + index]
                )
            else:
            ...
            module_block.add_module(
                f"detection_{index}",
                DetectionLayer(anchors=anchors, classes=block["classes"]),
            )
