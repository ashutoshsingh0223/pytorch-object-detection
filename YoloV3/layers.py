...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, image_size):
        ...
        x = x.view(bs, self.num_anchors, self.num_attrs, fm_h, fm_w).permute(
            0, 1, 3, 4, 2
        )
        x = x.reshape(bs, self.num_anchors * fm_h * fm_w, self.num_attrs).contiguous()

        if not self.training:
            cell_offsets = DetectionLayer.make_grid(
                grid_size=fm_h,
