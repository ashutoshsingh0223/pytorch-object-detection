...
def arg_parse():
    ...
    parser.add_argument("--annotations-file", help="Path to annotations file", type=str, default=None, required=False)
    ...
    parser.add_argument("--weights", help="weights file for yolo",
                        default="./weights/yolov3_coco_80_weights/yolov3.weights", type=str)
...
def draw_bbox(bboxes: 'Tensor', feature_map_res: Tuple[int, int], raw_image_path: Union[str, 'Path'],
              classes: List[str]):
...
if __name__ == '__main__':
    ...
    for batch_idx, (batch, target) in enumerate(val_loader):
        ...
        predictions = transform_detections(predictions, confidence, num_classes, nms_threshold)
        ...
        for image_idx in range(images_idx_start, min(image_idx_end+1, len(val_loader.dataset))):
